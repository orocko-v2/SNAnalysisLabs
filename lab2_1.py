...
def lab2_2():
    ...
    nx_graph.add_nodes_from([(user['id'], {'label': user['first_name'], 'color': color_map[user['sex']], 'sex': user['sex']}) for user in users])
    ...
    for node in nx_graph.nodes:
        nodes.append((node, nx_graph.nodes.get(node)['label']))
